  ...
  (mt/with-premium-features #{:table-data-editing}
    (mt/with-empty-h2-app-db
      (with-open [test-table1 (data-editing.tu/open-test-table!)
        ...
        (let [url            #(format "ee/data-editing/table/%d/webhook" %)
              req            #(mt/user-http-request-full-response %1 :get (url %2) {})
              create-url     "ee/data-editing/webhook"
              create-req     #(:body (mt/user-http-request-full-response :crowberto :post create-url {:table-id %}))
              status         (comp :status req)
          ...
          (testing "n tokens"
            (let [{token1 :token} (create-req table-id1)
                  {token2 :token} (create-req table-id2)
                  {token3 :token} (create-req table-id1)
    ...
    (mt/with-empty-h2-app-db
      (with-open [test-table (data-editing.tu/open-test-table!)]
        (let [url            #(format "ee/data-editing/webhook/%s" %)
              ...
              create-url     "ee/data-editing/webhook"
              create-req     #(:body (mt/user-http-request-full-response :crowberto :post create-url {:table-id %}))
              list-url       #(format "ee/data-editing/table/%d/webhook" %)
              list-req       #(:body (mt/user-http-request-full-response :crowberto :get (list-url %)))
              status         (comp :status req)
              table-id       @test-table
              {token :token} (create-req table-id)
          ...
          (testing "token does exist"
            ...
            (is (not-any? #{token} (map :token (:tokens (list-req table-id))))))
    ...
    (mt/with-empty-h2-app-db
      ...
      (with-open [test-table (data-editing.tu/open-test-table!
                              {:id [:int]
                               :v [:text]}
                              {:primary-key [:id]})]
        (let [url            #(format "ee/data-editing/webhook/%s/data" %)
              ;; todo not able to test with anon
              req            #(mt/user-http-request-full-response
                               :crowberto :post (url %1)
                               {:request-options {:body (.getBytes (json/encode %2))}})
              status         (comp :status req)
              create-url     "ee/data-editing/webhook"
              create-req     #(:body (mt/user-http-request-full-response :crowberto :post create-url {:table-id %}))
              delete-url     #(format "ee/data-editing/webhook/%s" %)
              delete-req     #(mt/user-http-request :crowberto :delete (delete-url %))
              table-id       @test-table
              {token :token} (create-req table-id)
              not-a-token    (str (random-uuid))]
          (testing "token does not exist"
